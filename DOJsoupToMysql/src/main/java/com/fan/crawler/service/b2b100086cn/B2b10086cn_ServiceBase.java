 ...
 */
public abstract class B2b10086cn_ServiceBase implements Runnable {
    private static Logger logger = LoggerFactory.getLogger(B2b10086cn_ServiceBase.class);
    protected SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

    @Autowired
    protected RawBidDAO bidDAO;
    ...
    public void run() {
        ...
        for (int page = 1; page <= max; page++) {
            logger.info(getServiceType()+":第" + page + "页");
            ...
            for (int rowNum = 2; rowNum < size; rowNum++) {
                ...
                // 获取是否重复
                boolean isdup = false;
                isdup = bidDAO.existById(rawBidInfo.getId());
